...
aws s3 cp /var/opt/tableau/tableau_server/data/tabsvc/files/backups/$TODAY.tsbak s3://$BUCKET_NAME$APP/$TODAY-backup.tsbak;
...
aws s3 cp "/tmp/backup/$TODAY.json" s3://$BUCKET_NAME$APP/$TODAY-settings.json;
